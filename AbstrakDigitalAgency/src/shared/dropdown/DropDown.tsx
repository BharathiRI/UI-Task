...
/**
 *
 * @param List - Array of Dropdown content
 * @param view - indicated it's Mobile or Desktop
 * @returns Dropdown
 */
      ...
      {List?.map((items: any, index: number) => (
        <Styled.DropdownList view={view} key={index}>
          <Styled.DropdownTextBox view={view}>
            <Styled.DropdownText>{items.text}</Styled.DropdownText>
            <svg
              stroke="currentColor"
              fill="currentColor"
              viewBox="0 0 320 512"
          ...
          <Styled.DropdownSubMenu view={view}>
            {items.subMenus?.map((menu: string, index: number) => (
              <Styled.DropdownSubMenuList view={view} key={index}>
                <Styled.DropdownSubText>{menu}</Styled.DropdownSubText>
